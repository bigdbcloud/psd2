...
public class TransactionRequestServiceImpl implements TransactionRequestService
{
	...
	@Autowired
	private TransactionStatementService txnStmtService;

	@Autowired
	private BankAccountDetailsService bankAccDetailsService;
	...
	private void postTransactionToBeneficiary(TxnRequestDetails txnReq, String date)
	{
		...
		try
		{
			BankAccountDetails to = bankAccDetailsService.getBankAccountDetails(txnReq.getBody().getTo().getBankId(),
			...
			if (to != null)
			{
				...
				bankAccDetailsService.updateBalance(to.getBankId(), to.getId(), newToBalance);
				...
				txnStmtService.createTransaction(txn);
			}
		}
	...
	{
		postTransactionToBeneficiary(txnReq, TxnRequestDetails.DATE_FORMAT.format(new Date()));
	}
	...
	private void postTransactionToPayer(TxnRequestDetails txnReq, String date)
	{
		...
		try
		{
			...
			BankAccountDetails from = bankAccDetailsService.getBankAccountDetails(txnReq.getFrom().getBankId(),
			...
			bankAccDetailsService.updateBalance(from.getBankId(), from.getId(), newFromBalance);
	...
	{
		postTransactionToPayer(txnReq, TxnRequestDetails.DATE_FORMAT.format(new Date()));
	}
			...
			String txnType)
	{
		...
		if (!useKafka && TxnRequestDetails.TXN_STATUS_PENDING.equalsIgnoreCase(txnRequest.getStatus()))
		{
			// Hack to bypass Txn Processing
			postTransactionToBeneficiary(txnRequest);
			postTransactionToPayer(txnRequest);
			txnRequest.setStatus(TxnRequestDetails.TXN_STATUS_COMPLETED);
			txnRequest.setEndDate(TxnRequest.DATE_FORMAT.format(new Date()));
		}

		txnRequest = mtrdr.save(txnRequest);

		if (useKafka && TxnRequestDetails.TXN_STATUS_PENDING.equalsIgnoreCase(txnRequest.getStatus()))
		{
			kmp.publishMessage(topic, txnRequest.getId(), txnRequest.toString());
		}

		// send subscription challenge answer to user registered Mobile No
		if (txnRequest.getChallenge() != null)
		{
			...
			if (userMobNo != null)
			{
				try
				{
					messageService.sendMessage(bankSubscribeRspnsNo, userMobNo,
							MessageFormat.format(Challenge.CHALLENGE_TXN, txnRequest.getChallenge().getAnswer()));
				}
				catch (TwilioRestException e)
				{
					logger.error("Failed to send challenge answer for transaction request : {}", txnRequest.getId());
				}
			}
		}
		return txnRequest;
			...
			String accountId, String txnType, String txnReqId, ChallengeAnswer t)
	{
		...
		// Hack to bypass Txn Processing
		if (!useKafka && tdb.getStatus().equals(TxnRequestDetails.TXN_STATUS_PENDING))
		{
			postTransactionToBeneficiary(tdb);
			postTransactionToPayer(tdb);
			tdb.setStatus(TxnRequestDetails.TXN_STATUS_COMPLETED);
			tdb.setEndDate(TxnRequest.DATE_FORMAT.format(new Date()));
		}

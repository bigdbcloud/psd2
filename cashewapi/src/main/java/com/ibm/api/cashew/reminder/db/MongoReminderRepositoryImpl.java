...
public class MongoReminderRepositoryImpl implements MongoReminderCustomRepository {
	...
	public List<Reminder> getReminders(String userId, String fromDate, String toDate) {
		...
		if (fromDate == null || toDate==null) {
			...
			cd = where("createdBy").is(userId)
					.and("reminderDate").gte(fromDate);
		}
		else
		{
			fromDate=Reminder.DATE_FORMAT.format(fromDate);	
			toDate=Reminder.DATE_FORMAT.format(toDate);
			
			cd = where("createdBy").is(userId)
					.and("reminderDate").lte(toDate).gte(fromDate);
		}
